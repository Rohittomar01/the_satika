...
const ProductsList = () => {
  ...
  const handleProcessRowUpdate = (newRow) => {
    setProducts((prevProducts) =>
      prevProducts.map((row) => (row.product_id === newRow.product_id ? newRow : row))
    );
  ...
  const confirmDelete = async () => {
    try {
      ...
      const updatedProducts = products.filter((row) => row.product_id !== deleteId);
  ...
  const columns = [
    ...
    { field: 'product_description', headerName: 'Description', width: 300, editable: true },
    ...
    { field: 'new_arrival', headerName: 'New Arrival', width: 150, editable: true },
    { field: 'top_selling', headerName: 'Top Selling', width: 150, editable: true },
    ...
    <div style={{ height: 600, width: '90%', marginTop: "10%" }}>
      {products.length > 0 ? (
       ...
       <div style={{height:"80vh",display:"flex",justifyContent:"center"}}>
         <p style={{color:"red",fontWeight:"bolder",fontSize:"1.5em"}}>No products available</p>
       </div>
      )}
    </div>
  );
